...
def listar_todas_caixas():
    ...
    for c in caixas:
        ...
        if c.prateleira:
            localizacao = f"Prateleira ID {c.prateleira.id} (Nível: {c.prateleira.nivel}, Coluna: {c.prateleira.coluna})"
        else:
...
def listar_caixas_de_prateleira():
    ...
    if caixas:
        for c in caixas:
            ...
            localizacao = f"Nível: {prateleira.nivel}, Coluna: {prateleira.coluna}"
            data_elim = c.data_eliminacao.strftime('%d/%m/%Y') if c.data_eliminacao else "Permanente"
...
def menu_caixa():
    while True:
        ...
        print("6. Importar caixas de arquivo CSV")
        print("7. Voltar")
        opcao = input("Escolha uma opção: ")
        ...
        if opcao == '1':
            ...
            prateleiras = repo_prateleira.get_all()
            if not unidades:
                print("Nenhuma unidade encontrada. Crie antes de adicionar caixas.")
                continue
            ...
            # --- Escolha da prateleira (com opção de deixar em branco) ---
            prateleira = None
            if prateleiras:
                print("\n--- Prateleiras disponíveis ---")
                for p in prateleiras:
                    print(f"{p.id} - Setor: {p.setor}, Corredor: {p.corredor}, Coluna: {p.coluna}, Nível: {p.nivel}")
                print("Pressione ENTER para deixar sem prateleira.")
                prateleira_id = input("Escolha o ID da prateleira: ").strip()
                if prateleira_id != "":
                    try:
                        prateleira_id = int(prateleira_id)
                        prateleira = repo_prateleira.get_by_id(prateleira_id)
                        if not prateleira:
                            print("Prateleira não encontrada. Caixa será cadastrada como avulsa.")
                    except ValueError:
                        print("Entrada inválida. Caixa será cadastrada como avulsa.")
            else:
                print("Nenhuma prateleira cadastrada. Caixa será cadastrada como avulsa.")
            ...
            if prateleiras_invalidas:
                print(f"Atenção: {len(prateleiras_invalidas)} prateleira(s) ignorada(s) por conterem valores não numéricos em coluna ou nível.")

            # ordenar por corredor (string), coluna (int), nível (int)
            prateleiras_setor = sorted(prateleiras_validas, key=lambda p: (p.corredor, int(p.coluna), int(p.nivel)))
            ...
            for caixa in caixas_avulsas:
                alocada = False
                for p in prateleiras_setor: # Itera sobre cada prateleira do setor
                    # Lógica para encontrar um espaço vago na prateleira p
                    try:
                        capacidade_niveis = int(p.nivel)
                        capacidade_colunas = int(p.coluna)
                    except (ValueError, TypeError):
                        # Pula prateleiras com dados de capacidade inválidos (ex: texto)
                        continue

                    # Mapeia todos os slots (nível, coluna) que já estão ocupados nesta prateleira
                    slots_ocupados = set()
                    for c_existente in getattr(p, 'caixas', []):
                        if c_existente.nivel is not None and c_existente.coluna is not None:
                            slots_ocupados.add((int(c_existente.nivel), int(c_existente.coluna)))

                    # Procura pelo primeiro slot vazio, iterando por todos os níveis e colunas
                    slot_encontrado = False
                    for n_atual in range(1, capacidade_niveis + 1):
                        for c_atual in range(1, capacidade_colunas + 1):
                            if (n_atual, c_atual) not in slots_ocupados:
                                # Encontrou um slot vazio! Aloca a caixa aqui.
                                caixa.prateleira = p
                                caixa.nivel = n_atual
                                caixa.coluna = c_atual
                                repo_caixa.update(caixa)
                                print(f"Caixa {caixa.numero_caixa} movida para Prateleira ID {p.id} (Setor: {p.setor}, Corredor: {p.corredor}, Coluna: {c_atual}, Nível: {n_atual})")
                                alocada = True
                    ...
                    if alocada:
                        break # Sai do loop de prateleiras, pois a caixa atual já foi alocada
